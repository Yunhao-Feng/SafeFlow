...
- Summarizing memory when token count exceeds threshold (100M tokens)
...
class ContextManagerAgent:
    ...
    def __init__(self, config, item_id: str, default_agent_name: str = "default") -> None:
        ...
        self.token_threshold = 9_000_000  # 100K tokens threshold for summarization (about 200 pages of text)
    ...
    def notify_turn_tokens(self, prompt_tokens: int) -> None:
        ...
        if prompt_tokens > self.token_threshold:
            context_console.print(f"🔥 High token usage detected: {prompt_tokens} tokens (threshold: {self.token_threshold})", style="yellow")
    ...
    def get_summarized_messages(self) -> List[Dict[str, Any]]:
        ...
        Strategy:
        - Keep recent turns (last 3-10 turns)
        - Keep initial turns (first 2-3 turns)
    ...
    def update_work_context(self, work_root: str = None, task_description: str = None,
        ...
        if initialization_status:
            self.current_state["initialization_status"] = initialization_status
        ...
        self.trace_track.add_message({
            "role": "system",
            "content": f"Context updated: work_root={work_root}, task={task_description}, status={initialization_status}"
        })
    ...
    def get_context_summary_for_agent(self) -> str:
        ...
        summary_parts = [
            f"🎯 Current Task: {context.get('current_task', 'None specified')}",
            f"📁 Work Root: {context.get('work_root', 'Not set')}",
            f"🔄 Status: {context.get('initialization_status', 'Unknown')}",
        ]
    ...
    def needs_context_reminder(self) -> bool:
        ...
        # Remind every 10 turns if work_root is set, or every 5 turns if not initialized
        if self.current_state.get('work_root'):
            return self.turn_count > 0 and self.turn_count % 10 == 0
        elif self.current_state.get('initialization_status') != 'completed':
